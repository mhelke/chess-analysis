...
#' @export
chessdata <- function(username, year, month, nmonths = 1) {
    ...
    chess <- chess %>%
      select(url, Date, time_control, time_class, rules, White, white_rating,
    ...
    if(na_val){
      remove_rows_index <- which(is.na(chess$url), arr.ind= FALSE, useNames = TRUE)
      chess <- chess %>%
        slice(-remove_rows_index)
    }
